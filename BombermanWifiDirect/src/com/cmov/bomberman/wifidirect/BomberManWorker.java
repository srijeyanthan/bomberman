...
public class BomberManWorker implements Runnable,IMoveableRobot,IExplodable,IUpdatableScore {
    ...
    public void processData(Server server, SocketChannel socketchannel,
			byte[] data, int count) {
    	String key="";
		key = socketchannel.socket().getRemoteSocketAddress().toString();
		if(!Server.clientList.containsKey(key))	
		{
		Server.clientList.put(key, new ServerDataEvent(server,socketchannel));
