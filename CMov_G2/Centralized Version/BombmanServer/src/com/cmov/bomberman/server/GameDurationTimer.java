...
class GameDurationTimer extends TimerTask {
	...
	public GameDurationTimer() {
		timer.schedule(this, Gameduration / 20, Gameduration / 20);
	}

	@Override
	public void run() {
		if (tick() >= 6) {
			System.out.println("[SERVER] Tesing .... "
					+ System.currentTimeMillis());
			// <1=P|18=Sri,900.Gureaya,9000.Bogdan,89>
			System.out
					.println("[SERVER] Game finished ...Bye Bye ...from server");
			String scoreupdatemsg = "<" + BombermanProtocol.MESSAGE_TYPE + "="
					+ BombermanProtocol.GAME_END_MESSAGE + "|"
					+ BombermanProtocol.GAME_STAT + "=";
			for (Map.Entry<Integer, String> entry : BomberManWorker.playernameidmap
					.entrySet()) {
				int totalscore = 0;
				System.out.println("[SERVER] Player id checking - "
						+ entry.getKey());
				if (BomberManWorker.globalScore.get(entry.getKey()) != null) {
					totalscore = BomberManWorker.globalScore
							.get(entry.getKey());
				}

				scoreupdatemsg += entry.getValue() + "," + totalscore + ".";
			}
			if (scoreupdatemsg.charAt(scoreupdatemsg.length() - 1) == '.') {
				scoreupdatemsg = scoreupdatemsg.replaceFirst(".$", "");
			}
			scoreupdatemsg += ">";

			System.out
					.println("[SERVER] Game finished . Sending update to all the clients -"
							+ scoreupdatemsg);
			...
			if (Server.deadPlayerlist.size() == Server.clientList.size()) {
				String scoreupdatemsg = "<" + BombermanProtocol.MESSAGE_TYPE
						+ "=" + BombermanProtocol.GAME_END_MESSAGE + "|"
						+ BombermanProtocol.GAME_STAT + "=";
				for (Map.Entry<Integer, String> entry : BomberManWorker.playernameidmap
					...
					System.out.println("[SERVER] Player id checking - "
							+ entry.getKey());
					if (BomberManWorker.globalScore.get(entry.getKey()) != null) {
						totalscore = BomberManWorker.globalScore.get(entry
								.getKey());
					}
